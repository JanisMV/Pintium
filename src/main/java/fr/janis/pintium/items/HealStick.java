...
public class HealStick extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        ...
        if(InputConstants.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), GLFW.GLFW_KEY_LEFT_SHIFT)){
            tooltip.add(new TranslatableComponent("tooltip.heal_stick.hold_shift"));
        } else {
            tooltip.add(new TranslatableComponent("tooltip.heal_stick.not_hold_shift"));
        }
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (!playerIn.getCooldowns().isOnCooldown(this)) {
            ...
            playerIn.addEffect(new MobEffectInstance(MobEffects.HEAL, 1, 4));
